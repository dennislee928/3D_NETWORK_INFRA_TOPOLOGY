...
本專案是一個基於 **React Three Fiber (3D)**、**Rust (Axum)** 與 **SDN (OpenDaylight)** 的網路拓樸視覺化系統。它能同時展示「實體網路層 (Physical)」、「虛擬網路層 (Virtual)」與「服務應用層 (Service)」，協助工程師直觀地理解複雜的基礎設施關聯。
...
- **後端 (SDN Adapter)**: Rust, Axum, Tokio, Reqwest (高效能 ODL 資料轉換器).
- **SDN 控制器**: OpenDaylight (ODL) - 負責 OpenFlow 拓樸管理與狀態收集.
...
- `backend/`: Rust 實作的 SDN Adapter，對接 ODL RESTCONF API 並轉換格式。
...
### A. 啟動所有容器服務
這會啟動 OpenDaylight、Rust Adapter、Mininet 容器以及前端介面。
```bash
docker-compose up -d --build
```
> **注意**：OpenDaylight 啟動與安裝 Feature 需要約 2-5 分鐘。您可以透過 `docker logs -f odl_controller` 查看進度。

### B. 進入 Mininet 建立模擬拓樸
當 ODL 啟動完成後（`docker ps` 顯示健康狀態），進入 Mininet 容器手動建立網路：
```bash
docker exec -it mininet_network bash
```
在容器內部執行 Mininet 命令：
```bash
# 建立一個包含 1 個 Switch、3 個 Host 的星型拓樸，並連向 ODL 控制器
...
2. **檢視實體層**：
   ...
   - 點擊節點可查看來自 ODL 的實體資訊（如 OpenFlow ID、連接埠數量等）。
3. **檢視動態風險**：
...
如果您只想修改前端或後端程式碼，不啟動 ODL：

### 1. 啟動 Rust 後端 (Adapter)
後端在無法連接 ODL 時會自動切換至 **Mock 模式**。
```bash
cd backend
cargo run
```
...
- **Physical Layer (底層, Y < 0)**: 顯示由 OpenDaylight 發現的實體 Switch 與 Host。
